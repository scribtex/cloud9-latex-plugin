...
return ext.register("ext/latex/compile", {
    ...
    addTabs : function() {
        ...
        // other.
        tabOutput.addEventListener("beforeswitch", this.deactivateEditorTabs);
        tabEditors.addEventListener("beforeswitch", this.deactivateOutputTabs);
    ...
    setPdfTabToPdf: function(url) {
        ...
        for (var i = 0; i < page.childNodes.length; i++) {
            page.childNodes[i].removeNode();
        }
        
        page.appendChild(new apf.pdf({
            skinset : "latex",
            source : url
        }));
    },
    ...
    setLogTabToLog: function(content) {
        ...
        var preElement = logContent.selectNodes("pre")[0];
        if (preElement)
            preElement.$ext.innerHTML = content;
        page.appendChild(logContent);
    },
